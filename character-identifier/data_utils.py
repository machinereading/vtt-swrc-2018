...
def load_video_vec(scene_embedding_dir):
    scene_embedding_map = {}
    scene_embedding = []
    scene_embedding.append(np.zeros((2048,),dtype=float))
    scene_embedding_cnt = 0
    ...
    for fname in fname_list:
        ...
        with open(fpath, 'rb') as handle:
            ...
            for item in data:
                scene_emb_key = str(item['st']) + '-' + str(item['en']) + '-' + fname
                if (scene_emb_key not in scene_embedding_map):
                    scene_embedding_cnt += 1
                    scene_embedding_map[scene_emb_key] = scene_embedding_cnt
                    scene_embedding.append(item['vec'])
                    
    
    return scene_embedding_map, scene_embedding


def load_data(filepath, entity_name_to_idx, with_ling_info=False, with_keys=None, logger=None, use_video=False):
    """
...
def _load_data(filepath, entity_name_to_idx, with_keys, with_ling_info=False, logger=None, use_video=False):
    """
    :param filepath: path of file containing the language data the model is trained/tested on
    :param entity_name_to_idx: a dictionary mapping names to indices
    :param with_lemma: True if storing the lemma of the token (default: False)
    ...
    * First tried NLTK's CONLL corpus reader, but this felt a bit clunky (and needs some preprocessing to ignore '#'-lines).
        ...
        # reader = conll.ConllCorpusReader('friends_data/Trial_and_Training_data_and_Entity_mapping',
                                           'friends.trial.episode_delim.conll')
    """
    cols = [0, 1, 2, 3, 9, 11, 12, 13, 14] if not with_ling_info else [0, 1, 2, 3, 4, 6, 9, 14]
    if not with_ling_info:
        names_cols = ['episode', 'scene', 'position', 'token', 'speaker', 'st', 'en', 'video_file', 'entity']
    else:
...
def create_input_vectors(data, vocabulary_id_to_word, vocabulary_word_to_id, scene_embedding_key_to_id={}):
    """
    ...
    for token_index in range(train_data_length):
        ...
        num_words[token_index] = len(speaker_ids) + 1 + 1  # plus 1 token at index 0, plus 1 for video scene info

        scene_key = str(data['st'][token_index]) + '-' + str(data['en'][token_index]) + '-' + data['video_file'][token_index]
        scene_vec_id = scene_embedding_key_to_id[scene_key] if scene_key in scene_embedding_key_to_id else 0
        token_speaker_ids.append(scene_vec_id)
    ...
    # -input_vec shape : (a,b), a = # of words, b = 1 + max # of speaker
    # ---[wor_id, speaker1_id, spaker2_id, ...]*#words
    # --- sample [18  248 -1 -1 -1], [3491 248 -1 -1 -1] ...
    # -targets shape : (a), a = # of words,
    # ---sample : [target_entity_id]
    print ('Input Vector Generated...')
