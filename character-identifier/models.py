...
class LSTM_basic(nn.Module):
    ...
    def __init__(self, args, padding_idx=-1):
        ...
        self.scene_emb_size=2048
        self.scene_hidden_size=args.video_hidden_size
        self.scene_emb_out_size=args.video_output_size


        #self.scene_emb_out_size=128
        if (self.use_video):
            self.scene_embedder = VideoSceneEmbedder(args, padding_idx=padding_idx)
        self.scene_ffnn = nn.Sequential(
            nn.Linear(self.scene_emb_size, self.scene_emb_out_size),
            nn.ReLU())
    ...
    def forward(self, padded_batch, desired_outputs_mask=None, scene_emb_list=[]):
        """
        ...
        if (self.use_video):
            scene_embeddings = self.scene_embedder(padded_batch)
            scene_feature = self.scene_ffnn(scene_embeddings)
        ...
        if (self.use_video):
            lstm_out = torch.cat((lstm_out, scene_feature), dim=2)
...
class VideoSceneEmbedder(nn.Module):
    def __init__(self, args, padding_idx=-1):
        super(VideoSceneEmbedder, self).__init__()

        self.padding_idx = padding_idx

        emb_weights = torch.Tensor(args.video_emb)
        self.scene_emb = nn.Embedding(emb_weights.shape[0], emb_weights.shape[1])
        self.scene_emb.weight.data = torch.Tensor(args.video_emb)

    def forward(self, padded_batch):
        # Compute embeddings of input
        embeddings = self._padded_embedding(self.scene_emb, padded_batch[:, :, 1])
        return embeddings
...
class UtteranceEmbedder(nn.Module):
    ...
    def forward(self, padded_batch):
        ...
        speaker_embeddings = self._padded_embedding(self.speaker_emb, padded_batch[:, :, 2:])
