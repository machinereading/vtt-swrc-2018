...
def yield_padded_batch_and_bounds(X, y, seq_bounds, batch_size, chunk_size, shuffle_data, use_cuda=False):
    ...
    for i, b in enumerate(seq_bounds_per_batch):
        ...
        if y != []:
            ...
            y_padded_sequences = [torch.cat([torch.unsqueeze(t,1), -1*torch.ones(max_seq_length -
                len(t),1).long()]) for t in y_sequences]
...
def get_indexed_predictions_with_targets(model, inputs, targets, sequence_bounds, cuda=False, collect_ensembles_preds=False, scene_emb_list=[]):
    """
    ...
    predictions, raw_scores = get_all_predictions_and_mean_loss(model, inputs, targets, sequence_bounds, batch_size=len(inputs), chunk_size=-1, loss_function=None, use_cuda=cuda, collect_ensembles_preds=collect_ensembles_preds, scene_emb_list=scene_emb_list)
...
def get_all_predictions_and_mean_loss(model, X_data, y_data, sequence_bounds, batch_size, chunk_size=-1, loss_function=None, use_cuda=False, collect_ensembles_preds=False, scene_emb_list=[]):
    """
    ...
    for X_batch, y_batch, bounds in yield_padded_batch_and_bounds(X_data, y_data, sequence_bounds, batch_size, chunk_size, shuffle_data=False, use_cuda=use_cuda):
        ...
        for X_batch_chunk, y_batch_chunk in zip(X_batch, y_batch):
            ...
            if isinstance(model, list):
                outputs = _apply_models_ensemble(model, padded_batch, desired_outputs_mask=mask, use_cuda=use_cuda, collect_ensembles_preds=collect_ensembles_preds, scene_emb_list=scene_emb_list)
            else:
                outputs = model(padded_batch, desired_outputs_mask=mask, scene_emb_list=scene_emb_list)
            # print(outputs)
...
def _apply_models_ensemble(models, padded_batch, desired_outputs_mask, use_cuda=False, collect_ensembles_preds=False, scene_emb_list=[]):
    """
    ...
    models_scores = []
    for model in models:
        model_outputs = model(padded_batch, desired_outputs_mask=desired_outputs_mask, scene_emb_list=scene_emb_list)
...
def train(model, X, y, train_sequence_bounds, validation_sequence_bounds, args, no_shuffle, logger, scene_emb_list=[]):
    """
    ...
    while epoch < args.epochs:
        ...
        if (epoch + 1) % args.test_every == 0:
            ...
            train_predictions, mean_train_loss = get_all_predictions_and_mean_loss(
                ...
                use_cuda=use_cuda, scene_emb_list=scene_emb_list)
